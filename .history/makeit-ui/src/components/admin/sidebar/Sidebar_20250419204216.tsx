...
export const AdmiSidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  const menuItems = [
    { id: "dashboard", label: "Dashboard", icon: <LayoutDashboard size={20} /> },
    { id: "users", label: "User Management", icon: <Users size={20} /> },
    { id: "vendors", label: "Vendor Management", icon: <Store size={20} /> },
    { id: "categories", label: "Category Management", icon: <FolderTree size={20} /> },
      ...
      <nav className="flex-1 py-4 overflow-y-auto">
        <ul className="space-y-1 px-3">
          {menuItems.map((item) => (
            <li key={item.id}>
              <button
                className={`flex items-center w-full px-3 py-2.5 text-sm rounded-md transition-colors relative ${
                  activeTab === item.id 
